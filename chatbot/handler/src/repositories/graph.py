...
from database.models import (
    Graph, GraphNode, GraphEdge, AvailableTool, NodeTool, 
...
from schemas.requests.graph import (
    ...
    NodeToolCreate, NodeToolUpdate, GraphExecutionCreate
)
...
class GraphRepository:
    ...
    def get_graph_with_details(self, graph_id: uuid.UUID) -> Optional[Graph]:
        return (
            self.db.query(Graph)
            .options(
                joinedload(Graph.nodes).joinedload(GraphNode.tools).joinedload(NodeTool.tool),
    ...
    def get_graphs(self, limit: int = 50, offset: int = 0, is_active: Optional[bool] = None) -> List[Graph]:
    ...
    def update_graph(self, graph_id: uuid.UUID, graph_data: GraphUpdate) -> Optional[Graph]:
    ...
    def create_node(self, graph_id: uuid.UUID, node_data: GraphNodeCreate) -> Optional[GraphNode]:
    ...
    def get_node_by_graph_and_node_id(self, graph_id: uuid.UUID, node_id: str) -> Optional[GraphNode]:
        return (
    ...
    def update_node(self, node_id: uuid.UUID, node_data: GraphNodeUpdate) -> Optional[GraphNode]:
    ...
    def create_edge(self, graph_id: uuid.UUID, edge_data: GraphEdgeCreate) -> Optional[GraphEdge]:
    ...
    def update_edge(self, edge_id: uuid.UUID, edge_data: GraphEdgeUpdate) -> Optional[GraphEdge]:
...
class ToolRepository:
    ...
    def get_tools(self, limit: int = 50, offset: int = 0, is_enabled: Optional[bool] = None) -> List[AvailableTool]:
        query = self.db.query(AvailableTool)
        if is_enabled is not None:
            query = query.filter(AvailableTool.is_enabled == is_enabled)
        return query.offset(offset).limit(limit).all()

    def update_tool(self, tool_id: uuid.UUID, tool_data: AvailableToolUpdate) -> Optional[AvailableTool]:
    ...
    def add_tool_to_node(self, node_id: uuid.UUID, tool_data: NodeToolCreate) -> Optional[NodeTool]:
    ...
    def update_node_tool(self, node_tool_id: uuid.UUID, tool_data: NodeToolUpdate) -> Optional[NodeTool]:
...
class GraphExecutionRepository:
    ...
    def create_execution(self, graph_id: uuid.UUID, execution_data: GraphExecutionCreate) -> GraphExecution:
    ...
    def get_executions_by_graph(self, graph_id: uuid.UUID, limit: int = 50, offset: int = 0) -> List[GraphExecution]:
        return (
            self.db.query(GraphExecution)
    ...
    def update_execution_status(self, execution_id: uuid.UUID, status: str, error_message: Optional[str] = None) -> Optional[GraphExecution]:
        db_execution = self.db.query(GraphExecution).filter(GraphExecution.id == execution_id).first()
        if not db_execution:
            return None
...
def get_graph_execution_repository(db: Session = next(get_db())) -> GraphExecutionRepository:
