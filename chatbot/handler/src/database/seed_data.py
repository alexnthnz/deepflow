...
from database.models import (
    AvailableTool, Graph, GraphNode, GraphEdge, NodeTool
)
...
def seed_default_tools(db: Session) -> dict:
    ...
    for tool_data in default_tools:
        try:
            ...
            existing_tool = db.query(AvailableTool).filter(
                AvailableTool.name == tool_data["name"]
            ).first()
...
def seed_default_graph(db: Session, tools: dict) -> Graph:
    ...
    existing_graph = db.query(Graph).filter(
        Graph.name == "Default Chat Graph"
    ).first()
    ...
    try:
        ...
        for edge_data in edges_data:
            ...
            db.add(edge)
            db.commit()
            logger.info(f"Created edge: {edge_data['from_node_id']} -> {edge_data['to_node_id']}")
